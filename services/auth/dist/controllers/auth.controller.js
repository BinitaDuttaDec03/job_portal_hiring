...
export const registerUser = TryCatch(async (req, res, next) => {
  ...
  `;
    if (existingUser.length > 0) {
        throw new ErrorHandler(409, "User with this email id already exists");
    }
    ...
    `;
        ...
        INSERT INTO users (name, email, password, phone_number, role, bio, resume, resume_public_id) VALUES (
            ${name}, ${email}, ${hashedPassword}, ${phoneNumber}, ${role}, ${bio}, ${data.url}, ${data.public_id}
        ) RETURNING user_id, name, email, phone_number, role, bio, resume, created_at;
    `;
        registeredUser = user;
    }
    const token = jwt.sign({ id: registeredUser?.user_id }, process.env.JWT_SECRET, {
        expiresIn: "15d",
    });
    res.json({
...
export const loginUser = TryCatch(async (req, res, next) => {
    ...
    const user = await sql `
    SELECT 
      u.user_id, u.name, u.password, u.phone_number, u.role, u.bio, u.resume, u.profile_pic, u.subscription, ARRAY_AGG(s.name) FILTER (WHERE s.name IS NOT NULL) AS skills 
